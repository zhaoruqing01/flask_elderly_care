...
class AdvancedPredictor:
    ...
    def prepare_training_data(self):
        ...
        # 读取服务记录（使用 service_record 表）
        service_df = pd.read_sql('SELECT * FROM service_record', conn)
        # 读取老人信息，用于添加年龄相关特征
        senior_df = pd.read_sql('SELECT elderly_id, age FROM elderly', conn)
        conn.close()
        ...
        # 合并老人信息（通过 elderly_id）
        service_df = pd.merge(service_df, senior_df, left_on='elderly_id', right_on='elderly_id', how='left')
    ...
    def predict_demand(self, community_id, service_type, days=30):
        ...
        # 查询历史服务记录
        query = f"""
        SELECT sr.service_date, SUM(sr.duration) as demand, AVG(e.age) as avg_age, AVG(sr.satisfaction) as satisfaction
        FROM service_record sr
        LEFT JOIN elderly e ON sr.elderly_id = e.elderly_id
        WHERE sr.community_id = '{community_id}' AND sr.service_type = '{service_type}'
        GROUP BY sr.service_date
