...
def clean_health_records():
    ...
    try:
        ...
        senior_df = pd.read_sql('SELECT id, elderly_id, age FROM elderly', conn)
        ...
        # health_record uses elderly_id
        merged_df = pd.merge(health_df, senior_df, left_on='elderly_id', right_on='elderly_id', suffixes=('', '_senior'))
        ...
        # 应用日期标准化函数
        # 标准化 record_date 字段
        if 'record_date' in cleaned_df.columns:
            cleaned_df['record_date'] = cleaned_df['record_date'].apply(normalize_date)
        elif 'date' in cleaned_df.columns:
            cleaned_df['record_date'] = cleaned_df['date'].apply(normalize_date)
        ...
        insert_data = cleaned_df[[
            'elderly_id', 'record_date', 'sbp', 'dbp', 'blood_sugar', 'heart_rate', 'health_status'
        ]].values.tolist()
...
def clean_service_logs():
    ...
    try:
        ...
        insert_data = service_df[[
            'elderly_id', 'service_date', 'service_type', 'duration', 'satisfaction', 'community_id'
        ]].values.tolist()
        cursor.executemany(
            'INSERT INTO service_record (elderly_id, service_date, service_type, duration, satisfaction, community_id) VALUES (?, ?, ?, ?, ?, ?)',
            insert_data
        )
