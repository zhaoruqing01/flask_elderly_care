...
def init_database():
    ...
    # 创建老人基本信息表（统一为 elderly）
    cursor.execute('''
    CREATE TABLE IF NOT EXISTS elderly (
        id INTEGER PRIMARY KEY,  -- 自增主键
        elderly_id TEXT UNIQUE,  -- 老人外部ID，如 E0001
        name TEXT,               -- 姓名
        age INTEGER,             -- 年龄
...
def generate_and_insert():
    ...
    # 清空现有数据（同时兼容旧表名）
    cursor.execute('DROP TABLE IF EXISTS service_log')
    cursor.execute('DROP TABLE IF EXISTS senior')
    cursor.execute('DELETE FROM service_record')
    cursor.execute('DELETE FROM health_record')
    cursor.execute('DELETE FROM elderly')
    cursor.execute('DELETE FROM prediction_result')
    conn.commit()
    
    # 生成老人数据
    seniors = generate_senior_data()
    # 批量插入老人数据（构造 elderly_id 和姓名）
    elderly_rows = []
    for item in seniors:
        idx, age, community = item
        elderly_id = f'E{idx:05d}'
        name = fake.name()
        elderly_rows.append((idx, elderly_id, name, age, community))
    cursor.executemany('INSERT INTO elderly (id, elderly_id, name, age, community_id) VALUES (?, ?, ?, ?, ?)', elderly_rows)
    conn.commit()
    ...
    for senior in seniors:
        ...
        # 生成健康记录（使用 elderly_id）
        elderly_id = f'E{senior_id:05d}'
        health_records = generate_health_records(elderly_id, age)
        if health_records:
            # 批量插入健康记录，注意字段顺序
            cursor.executemany('INSERT INTO health_record (elderly_id, record_date, sbp, dbp, blood_sugar, heart_rate, health_status) VALUES (?, ?, ?, ?, ?, ?, ?)', health_records)
        
        # 生成服务记录（使用 elderly_id）
        service_logs = generate_service_logs(elderly_id, community_id, age)
        if service_logs:
            # 批量插入服务记录
